...
ids = Style.connection.select_values("select style_id from styles join style_codes on style_id = styles.id where subcategory = 'tumblr' and !obsolete")
...
ids.each do |id|
	...
	doc.document_queries.reverse.each do |dq|
		...
		next if dq.url_functions.find{ |uf|
			...
			next v == 'domain' && ['tumblr.com', 'www.tumblr.com'].include?(v)
		}
		# find http rules
		http_functions = dq.url_functions.select{ |uf|
			n, v = function_info(uf)
			next ['url', 'url-prefix'].include?(n) && v.starts_with?('http://www.tumblr.com') && v != 'http://www.tumblr.com/goodies'
		}
